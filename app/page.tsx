...
function Logo({ size = 'text-xl' }: { size?: string }) {
  return (
    <span className={`font-extrabold tracking-tight ${size}`} style={{ color: 'var(--text)' }}>
  ...
  return (
    <div className={`flex ${meu ? 'justify-end' : 'justify-start'}`}>
      <div
        ...
        style={meu ? { background: '#25995c' } : { background: 'var(--card)', color: 'var(--text)', border: '1px solid var(--border)' }}
      >
        {children}
      </div>
    </div>
      ...
      <header className="sticky top-0 z-40 backdrop-blur border-b" style={{ background: 'var(--bg)', borderColor: 'var(--border)' }}>
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setDark(d => !d)}
              className="rounded-lg w-9 h-9 grid place-items-center border text-sm"
            ...
            <div className="rounded-[2rem] p-3 shadow-xl border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
              <div className="rounded-[1.5rem] overflow-hidden" style={{ background: dark ? '#0b141a' : '#eae6df' }}>
                ...
                <div className="p-3 space-y-2 min-h-[320px]">
                  ...
                  <Bolha de="ia">Fechado! ✅ Degradê + barba, amanhã às <b>16h30</b>. Te lembro na véspera. Até lá! 🙌</Bolha>
                </div>
              </div>
            </div>
      ...
      <section className="border-y" style={{ background: 'var(--soft)', borderColor: 'var(--border)' }}>
        <div className="max-w-6xl mx-auto px-4 py-16 grid md:grid-cols-2 gap-10 items-center">
          <div className="order-2 md:order-1 grid grid-cols-2 gap-3">
            {[
              { t: 'Receita no mês', v: 'R$ 8.450', c: 'var(--text)' },
              { t: 'Novos clientes pela IA', v: '27', c: AZUL },
              { t: 'Resgatados pela IA', v: '14', c: '#1f7a52' },
              { t: 'Valor gerado pela IA', v: 'R$ 3.900', c: AZUL },
            ].map((k, i) => (
              <div key={i} className="rounded-2xl p-4 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
                <p className="text-xs" style={{ color: 'var(--muted)' }}>{k.t}</p>
